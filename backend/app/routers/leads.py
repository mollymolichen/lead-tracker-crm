"""Expose API endpoints for retrieving leads and their call history."""
...
def get_leads(db: Session = Depends(get_db)):
    """Return all leads ordered by referral date and Salesforce ID."""
...
def get_lead_calls(lead_id: str, db: Session = Depends(get_db)):
    """Return the call history for a lead in reverse chronological order."""
    return db.scalars(
