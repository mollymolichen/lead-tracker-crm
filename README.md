...
Instead of updating two data sources (Salesforce, Talkdesk), we can initiate Talkdesk

#### Representatives run phone calls differently
Instead of relying on a spreadsheet or human memory to run phone calls, we will standardize the phone call scripts and initiate calls from a single location.
